...
function generateNearbyPins([lng, lat], count = 5) {
  ...
  const baseAngles = Array.from({ length: count }, (_, i) => (2 * Math.PI * i) / count);
  ...
  baseAngles.forEach((angle, i) => {
    ...
    const dLng = (distanceKm / earthRadius) * (180 / Math.PI) * Math.cos(angle) / Math.cos(lat * Math.PI / 180);
...
const MapboxMap = ({ lat, lng }) => {
  ...
  useEffect(() => {
    ...
    if (!animatedBikeMarkerRef.current) {
      ...
      animatedBikeMarkerRef.current = new mapboxgl.Marker(markerDiv).setLngLat(animatedBikePos).addTo(map.current);
    } else {
  ...
  function addOrUpdateHomePin(pos, i) {
    ...
    createRoot(el).render(
      <HomeIcon sx={{ fontSize: 18, color: '#1976d2' }} />
    );
  ...
  return (
    <div style={{ width: '100vw', height: 'calc(100vh - 220px)' }}>
